...
## ชั้นกฎพื้นฐาน (ทำงานได้แม้ AI ล่ม)

`apps/api/src/rewrite/rules/` เป็นชั้นแก้ข้อความด้วยกฎล้วน ๆ ไม่เรียก AI และไม่มีโควตา

เมื่อ AI ใช้ไม่ได้ (โควตาหมด, timeout, ยังไม่ได้ตั้ง API key) ระบบจะแก้ให้เท่าที่กฎทำได้
แล้วตอบกลับพร้อม `source: "rules"` หน้าเว็บจะขึ้นคำเตือนว่ายังไม่ได้ผ่าน AI ให้ตรวจทานเพิ่ม
...
แก้ `thai-rules.ts` ไฟล์เดียว แต่ละกฎมี `pattern`, `replacement`, `reason`
